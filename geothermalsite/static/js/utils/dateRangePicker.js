...
function invalidDate(date) {
    for (var i = 0; i < disabledRanges.length; i++) {
        if (
            date >= moment(disabledRanges[i].start) &&
            date <= moment(disabledRanges[i].end)
        ) {
            return true;
        }
    }
        ...
        .daterangepicker({
            ...
            ranges: {
                "Last 7 Available Days": [
                    moment(dataEndDate).subtract(6, "days"),
                    moment(),
                ],
                "Last 30 Available Days": [
                    moment(dataEndDate).subtract(29, "days"),
                    moment(),
                ],
                "Last 3 Available Months": [
                    moment(dataEndDate).subtract(3, "months"),
                    moment(),
                ],
                "Last Available Year": [
                    moment(dataEndDate).subtract(1, "years"),
                    moment(),
                ],
            },
        })
...
$("#id_temperatureProfileTimeSelector")
    .daterangepicker({
        timePicker: true,
    ...
    .on("apply.daterangepicker", function (ev, picker) {
        cacheInput(
            "temperatureProfileTimeSelector",
            $("#id_temperatureProfileTimeSelector")[0].value
        );
    });

$("#id_tempVsTimeDateRange")
    .daterangepicker({
        isInvalidDate: invalidDate,
        minDate: dataStartDate,
        maxDate: dataEndDate,
...
$("#id_tempVsTimeDateRange").value =
    moment(dataEndDate).subtract(30, "days").format("MM/DD/YYYY") +
    " - " +
    moment(dataEndDate).format("MM/DD/YYYY");
...
$("#id_temperatureProfileDateRange").value =
    moment(dataEndDate).subtract(30, "days").format("MM/DD/YYYY") +
    " - " +
    moment(dataEndDate).format("MM/DD/YYYY");
